...
commit-release:
	...
	git commit -m "chore: $$(cat release-notes/bump.txt) version bump v$$(cat release-notes/version.txt) $(CI_SKIP)"
	git push
...
tag-release:
	...
	git push -d $(GIT_REMOTE_NAME) v$$(cat release-notes/version.txt) || true

	# Add tag to the remote
	git tag v$$(cat release-notes/version.txt)
	git push $(GIT_REMOTE_NAME) v$$(cat release-notes/version.txt)
