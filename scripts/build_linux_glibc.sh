...
# Build linux/amd64
docker build . -f ./dockerfiles/Dockerfile_linux_glibc -t cli-linux-glibc-builder-image --build-arg GO_VERSION=$(cat .go-version)
...
if [ "$architecture" == 'x86_64' ]; then
  docker build . -f ./dockerfiles/Dockerfile_linux_glibc_arm64_from_amd64 -t cli-linux-glibc-arm64-builder-image  --build-arg GO_VERSION=$(cat .go-version)
else
  docker build . -f ./dockerfiles/Dockerfile_linux_glibc_arm64 -t cli-linux-glibc-arm64-builder-image  --build-arg GO_VERSION=$(cat .go-version)
fi
