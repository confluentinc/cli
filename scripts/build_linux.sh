...
# Build linux/amd64
docker build . --file ./docker/Dockerfile_linux_amd64 --tag cli-linux-amd64-builder-image --build-arg NFPM_PASSPHRASE="$NFPM_PASSPHRASE" # not finalized
...
if [ "$architecture" == 'x86_64' ]; then
  docker build . --file ./docker/Dockerfile_linux_arm64_from_amd64 --tag cli-linux-arm64-builder-image --build-arg NFPM_PASSPHRASE="$NFPM_PASSPHRASE" # not finalized
else
  docker build . --file ./docker/Dockerfile_linux_arm64 --tag cli-linux-arm64-builder-image --build-arg NFPM_PASSPHRASE="$NFPM_PASSPHRASE" # not finalized
fi
