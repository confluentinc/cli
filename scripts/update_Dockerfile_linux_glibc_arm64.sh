...
for tag in ${arm64_tags[@]}; do
  if [ $tag != "arm64-latest" ]; then
    ...
    docker build --no-cache --file ./docker/Dockerfile_linux_glibc_arm64_base --tag "050879227952.dkr.ecr.us-west-1.amazonaws.com/confluentinc/cli-ubuntu-base-arm64:$incremented_tag" . && \
