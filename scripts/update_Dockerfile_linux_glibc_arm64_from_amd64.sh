...
for tag in ${arm64_from_amd64_tags[@]}; do
  if [ $tag != "latest" ]; then
    ...
    docker build --no-cache --file ./docker/Dockerfile_linux_glibc_arm64_from_amd64_base --tag "050879227952.dkr.ecr.us-west-1.amazonaws.com/confluentinc/cli-ubuntu-base-arm64:$incremented_tag" . && \
