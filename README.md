...
In particular, the repository also contains all of the code for the on-prem "*RBAC CLI*," which is also built
...
$ make deps
$ make build
$ dist/cloud/$(go env GOOS)_$(go env GOARCH)/ccloud -h # for cloud CLI
$ dist/rbac/$(go env GOOS)_$(go env GOARCH)/confluent -h # for RBAC CLI
```
