...
type MetadataServiceOperationsApi interface {
	...
	/*
	 * Activenodes Returns all the nodes running the Metadata Service REST API. Clients are expected to round robin call to these endpoints if they don't set up a load balancer in front of the Metadata Service nodes. Callable by authenticated users.
	 *
	...
	/*
	 * MetadataClusterId Returns the ID of the Kafka cluster that MDS is running on.  Callable by LDAP users.
	 *
	 * @param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	 * @return string
	 */
...
func (a *MetadataServiceOperationsApiService) Activenodes(ctx _context.Context, protocol string) ([]string, *_nethttp.Response, error) {
	...
	localVarPath := a.client.cfg.BasePath + "/activenodes/{protocol}"
	localVarPath = strings.Replace(localVarPath, "{"+"protocol"+"}", _neturl.PathEscape(parameterToString(protocol, "")), -1)
...
func (a *MetadataServiceOperationsApiService) MetadataClusterId(ctx _context.Context) (string, *_nethttp.Response, error) {
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/metadataClusterId"
	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := _neturl.Values{}
	localVarFormParams := _neturl.Values{}
