...
func (a *PrivateRBACUIClusterVisibilityApiService) Visibility(ctx _context.Context, principal string, visibilityRequest []VisibilityRequest) (VisibilityResponse, *_nethttp.Response, error) {
	var (
		...
		localVarFileName     string
		localVarFileBytes    []byte
		localVarReturnValue  VisibilityResponse
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/security/1.0/lookup/principals/{principal}/visibility"
	localVarPath = strings.Replace(localVarPath, "{"+"principal"+"}", _neturl.PathEscape(parameterToString(principal, "")), -1)
	...
	// body params
	localVarPostBody = &visibilityRequest
	r, err := a.client.prepareRequest(ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, localVarFormFileName, localVarFileName, localVarFileBytes)
	if err != nil {
		return localVarReturnValue, nil, err
	}
