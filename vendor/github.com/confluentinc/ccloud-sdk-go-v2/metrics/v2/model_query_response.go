...
func (dst *QueryResponse) UnmarshalJSON(data []byte) error {
	var err error
        match := 0
        // try to unmarshal data into FlatQueryResponse
        err = json.Unmarshal(data, &dst.FlatQueryResponse)
        if err == nil {
                jsonFlatQueryResponse, _ := json.Marshal(dst.FlatQueryResponse)
                if string(jsonFlatQueryResponse) == "{}" { // empty struct
                        dst.FlatQueryResponse = nil
                } else {
                        match++
                }
        } else {
        ...
        // try to unmarshal data into GroupedQueryResponse
        err = json.Unmarshal(data, &dst.GroupedQueryResponse)
        if err == nil {
                jsonGroupedQueryResponse, _ := json.Marshal(dst.GroupedQueryResponse)
                if string(jsonGroupedQueryResponse) == "{}" { // empty struct
                        dst.GroupedQueryResponse = nil
                } else {
                        match++
                }
        } else {
        ...
        if match > 1 { // more than 1 match
                ...
                return fmt.Errorf("Data matches more than one schema in oneOf(QueryResponse)")
        } else if match == 1 {
                return nil // exactly one match
        } else { // no match
                return fmt.Errorf("Data failed to match schemas in oneOf(QueryResponse)")
        }
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src QueryResponse) MarshalJSON() ([]byte, error) {
	if src.FlatQueryResponse != nil {
		return json.Marshal(&src.FlatQueryResponse)
	}

	if src.GroupedQueryResponse != nil {
		return json.Marshal(&src.GroupedQueryResponse)
	}
...
func (obj *QueryResponse) GetActualInstance() (interface{}) {
	...
	if obj.FlatQueryResponse != nil {
		return obj.FlatQueryResponse
	}

	if obj.GroupedQueryResponse != nil {
		return obj.GroupedQueryResponse
	}
