...
func (dst *Filter) UnmarshalJSON(data []byte) error {
	var err error
        match := 0
        // try to unmarshal data into CompoundFilter
        err = json.Unmarshal(data, &dst.CompoundFilter)
        if err == nil {
                jsonCompoundFilter, _ := json.Marshal(dst.CompoundFilter)
                if string(jsonCompoundFilter) == "{}" { // empty struct
                        dst.CompoundFilter = nil
                } else {
                        match++
                }
        } else {
        ...
        err = json.Unmarshal(data, &dst.FieldFilter)
        if err == nil {
        ...
        // try to unmarshal data into UnaryFilter
        err = json.Unmarshal(data, &dst.UnaryFilter)
        if err == nil {
                jsonUnaryFilter, _ := json.Marshal(dst.UnaryFilter)
                if string(jsonUnaryFilter) == "{}" { // empty struct
                        dst.UnaryFilter = nil
                } else {
                        match++
                }
...
func (src Filter) MarshalJSON() ([]byte, error) {
	if src.CompoundFilter != nil {
		return json.Marshal(&src.CompoundFilter)
	}
	...
	if src.UnaryFilter != nil {
		return json.Marshal(&src.UnaryFilter)
	}
...
func (obj *Filter) GetActualInstance() (interface{}) {
	...
	if obj.CompoundFilter != nil {
		return obj.CompoundFilter
	}
	...
	if obj.UnaryFilter != nil {
		return obj.UnaryFilter
	}
