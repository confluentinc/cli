...
func (dst *FieldFilterValue) UnmarshalJSON(data []byte) error {
	var err error
        ...
        if match > 1 { // more than 1 match
                ...
                return fmt.Errorf("Data matches more than one schema in oneOf(FieldFilterValue)")
        } else if match == 1 {
                return nil // exactly one match
        } else { // no match
                return fmt.Errorf("Data failed to match schemas in oneOf(FieldFilterValue)")
        }
}
...
func (v NullableFieldFilterValue) Get() *FieldFilterValue {
	return v.value
}

func (v *NullableFieldFilterValue) Set(val *FieldFilterValue) {
	v.value = val
	v.isSet = true
}
...
func NewNullableFieldFilterValue(val *FieldFilterValue) *NullableFieldFilterValue {
	return &NullableFieldFilterValue{value: val, isSet: true}
}
