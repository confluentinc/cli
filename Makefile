...
coverage-unit:
	...
	@# Run unit tests.
      ifdef FORMAT
	@GO111MODULE=on GOPRIVATE=github.com/confluentinc go test -race -coverpkg=./... $$(go list ./... | grep -v vendor | grep -v test) $(UNIT_TEST_ARGS) -ldflags '-buildmode=exe' | go run digest.go $(FORMAT)
      else
