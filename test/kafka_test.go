...
func (s *CLITestSuite) TestKafka() {
	...
	tests := []CLITest{
		...
		{args: "kafka acl list --cluster lkc-acls-kafka-api", fixture: "kafka/acl/list-cloud.golden", env: []string{"XX_CCLOUD_USE_KAFKA_API=true"}},
		{args: "kafka acl list --cluster lkc-acls", fixture: "kafka/acl/list-cloud.golden", env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		{args: "kafka acl list --cluster lkc-acls -o json", fixture: "kafka/acl/list-json-cloud.golden", env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		{args: "kafka acl list --cluster lkc-acls -o yaml", fixture: "kafka/acl/list-yaml-cloud.golden", env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		{args: "kafka acl create --cluster lkc-acls --allow --service-account 7272 --operation READ --operation DESCRIBED --topic test-topic", fixture: "kafka/acl/invalid-operation.golden", wantErrCode: 1, env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		...
		{args: "kafka acl create --cluster lkc-acls --allow --service-account sa-12345 --operation READ --operation DESCRIBE --topic test-topic", fixture: "kafka/acl/create-service-account.golden", env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		...
		{args: "kafka acl create --cluster lkc-acls --allow --principal User:sa-12345 --operation WRITE --operation ALTER --topic test-topic", fixture: "kafka/acl/create-principal.golden", env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		{args: "kafka acl create --cluster lkc-acls --allow --service-account sa-54321 --operation READ --operation DESCRIBE --topic test-topic", fixture: "kafka/acl/invalid-service-account.golden", wantErrCode: 1, env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		{args: "kafka acl delete --cluster lkc-acls --allow --service-account sa-12345 --operation READ --operation DESCRIBE --topic test-topic", fixture: "kafka/acl/delete-cloud.golden", env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		{args: "kafka acl delete --cluster lkc-acls --allow --service-account sa-12345 --operation READ --operation DESCRIBE --topic test-topic", fixture: "kafka/acl/delete-cloud.golden", env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		{args: "kafka acl delete --cluster lkc-acls --allow --principal User:sa-12345 --operation WRITE --operation ALTER --topic test-topic", fixture: "kafka/acl/delete-cloud.golden", env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		{args: "kafka acl delete --cluster lkc-acls --allow --principal User:sa-12345 --operation WRITE --operation ALTER --topic test-topic", fixture: "kafka/acl/delete-cloud.golden", env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},

		{args: "kafka topic list --cluster lkc-kafka-api-topics", login: "cloud", fixture: "kafka/topic/list-cloud.golden", env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		{args: "kafka topic list --cluster lkc-topics", fixture: "kafka/topic/list-cloud.golden", env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		{args: "kafka topic list --cluster lkc-topics", fixture: "kafka/topic/list-cloud.golden", env: []string{"XX_CCLOUD_USE_KAFKA_API=true"}},
		...
		{args: "kafka topic list", login: "cloud", useKafka: "lkc-not-ready", fixture: "kafka/topic/list-cluster-not-ready.golden", wantErrCode: 1, env: []string{"XX_CCLOUD_USE_KAFKA_API=true"}},
		...
		{args: "kafka topic describe topic2", login: "cloud", useKafka: "lkc-describe-topic", fixture: "kafka/topic/describe-not-found-topic2.golden", wantErrCode: 1, env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		...
		{args: "kafka topic delete topic2", login: "cloud", useKafka: "lkc-delete-topic", fixture: "kafka/topic/delete-not-found-topic2.golden", wantErrCode: 1, env: []string{"XX_CCLOUD_USE_KAFKA_REST=true"}},
		...
		// Cluster linking
		{args: "kafka link create my_link --source-cluster-id lkc-describe-topic --source-bootstrap-server myhost:1234 --config-file " + getCreateLinkConfigFile(), fixture: "kafka/link/create-link.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka link list --cluster lkc-describe-topic", fixture: "kafka/link/list-link-plain.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka link list --cluster lkc-describe-topic -o json", fixture: "kafka/link/list-link-json.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka link list --cluster lkc-describe-topic -o yaml", fixture: "kafka/link/list-link-yaml.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka link describe --cluster lkc-describe-topic link-1", fixture: "kafka/link/describe-link-plain.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka link describe --cluster lkc-describe-topic link-1 -o json", fixture: "kafka/link/describe-link-json.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka link describe --cluster lkc-describe-topic link-1 -o yaml", fixture: "kafka/link/describe-link-yaml.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},

		{args: "kafka mirror list --cluster lkc-describe-topic --link link-1", fixture: "kafka/mirror/list-mirror.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka mirror list --cluster lkc-describe-topic --link link-1 -o json", fixture: "kafka/mirror/list-mirror-json.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka mirror list --cluster lkc-describe-topic --link link-1 -o yaml", fixture: "kafka/mirror/list-mirror-yaml.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka mirror list --cluster lkc-describe-topic", fixture: "kafka/mirror/list-all-mirror.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka mirror list --cluster lkc-describe-topic -o json", fixture: "kafka/mirror/list-all-mirror-json.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka mirror list --cluster lkc-describe-topic -o yaml", fixture: "kafka/mirror/list-all-mirror-yaml.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka mirror describe topic-1 --link link-1 --cluster lkc-describe-topic", fixture: "kafka/mirror/describe-mirror.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka mirror describe topic-1 --link link-1 --cluster lkc-describe-topic -o json", fixture: "kafka/mirror/describe-mirror-json.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka mirror describe topic-1 --link link-1 --cluster lkc-describe-topic -o yaml", fixture: "kafka/mirror/describe-mirror-yaml.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka mirror promote topic1 topic2 --cluster lkc-describe-topic --link link-1", fixture: "kafka/mirror/promote-mirror.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka mirror promote topic1 topic2 --cluster lkc-describe-topic --link link-1 -o json", fixture: "kafka/mirror/promote-mirror-json.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
		{args: "kafka mirror promote topic1 topic2 --cluster lkc-describe-topic --link link-1 -o yaml", fixture: "kafka/mirror/promote-mirror-yaml.golden", wantErrCode: 0, useKafka: "lkc-describe-topic"},
	}
