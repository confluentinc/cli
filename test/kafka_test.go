...
func (s *CLITestSuite) TestKafka() {
	...
	tests := []CLITest{
		...
		{args: "kafka link create bidirectional_link --remote-cluster lkc-describe-topic --remote-api-key api-key-123 --remote-api-secret api-secret-123 --remote-bootstrap-server myhost:1234 --config-file " + getCreateBidirectionalLinkConfigFile(), fixture: "kafka/link/create-bidirectional-link.golden", useKafka: "lkc-describe-topic"},
