...
// and "state=<value>" expressions, as passed by the CLI's --filter flag.
...
func applicationMatchesFilter(app cmfsdk.FlinkApplication, key, value string) bool {
	...
	case "state":
		...
		// CMF matches state case-sensitively against the upper-case job-state enum; the CLI
		// upper-cases the user's "state=" value before sending, so the match here is exact.
