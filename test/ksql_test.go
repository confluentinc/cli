...
func (s *CLITestSuite) TestKSQL() {
	...
	clusterTests := []CLITest{
		...
		{args: "ksql cluster create test_ksql_json --cluster lkc-processLogFalse --api-key key --api-secret secret --log-exclude-rows -o json", fixture: "ksql/cluster/create-result-json-log-exclude-rows.golden"},
...
func (s *CLITestSuite) TestKsqlClusterConfigureAcls() {
	tests := []CLITest{
		...
		{args: "ksql cluster configure-acls lksqlc-12345 --cluster lkc-abcde --unsafe-trace", fixture: "ksql/cluster/configure-acls.golden"},
