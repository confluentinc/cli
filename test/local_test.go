...
func (s *CLITestSuite) TestLocalHelpCommands() {
	...
	if runtime.GOOS == "windows" {
		tests = []CLITest{
			...
			{args: "local", fixture: "local-help1-windows.golden"},
			{args: "help local", fixture: "local-help1-windows.golden"},
			{args: "local --help", fixture: "local-help1-windows.golden"},
			...
			{args: "help local list", fixture: "local-help1-windows.golden"},
			// But if we call it this way, we can see help for specific local subcommands.
			{args: "local list --help", fixture: "local-help2-windows.golden"},
			// We only have help 2 command levels deep. "local list plugins --help" shows the same as "local list --help"
			{args: "local list plugins --help", fixture: "local-help2-windows.golden"},
		}
	} else {
		tests = []CLITest{
			// These should all be equivalent
