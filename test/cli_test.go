...
func serve(t *testing.T, kafkaAPIURL string) *httptest.Server {
	...
	router.HandleFunc("/api/ksqls/lksqlc-ksql1/", func(w http.ResponseWriter, r *http.Request) {
		...
		create, err := utilv1.MarshalJSONToBytes(&ksqlv1.GetKSQLClusterReply{
			Cluster: ksqlCluster,
		})
		require.NoError(t, err)
		_, err = io.WriteString(w, string(create))
		require.NoError(t, err)
	})
...
func handleKSQLCreateList(t *testing.T) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "POST" {
			response, err := utilv1.MarshalJSONToBytes(&ksqlv1.GetKSQLClusterReply{
			...
			listResponse, err := utilv1.MarshalJSONToBytes(&ksqlv1.GetKSQLClustersReply{
				Clusters: []*ksqlv1.KSQLCluster{ksqlCluster1, ksqlCluster2,},
			})
			require.NoError(t, err)
			_, err = io.WriteString(w, string(listResponse))
			require.NoError(t, err)
		}
	}
}
