...
func (s *CLITestSuite) TestFlinkConnectionCreateSuccess() {
	tests := []CLITest{
		...
		{args: "flink connection create my-connection --cloud aws --region eu-west-1 --type rest --endpoint https://api.example.com --oauth-token-endpoint https://api.example.com/auth --oauth-client-id clientId --oauth-client-secret secret --oauth-scope test_scope", fixture: "flink/connection/create/create-rest.golden"},
