...
func (s *CLITestSuite) TestStreamShare() {
	tests := []CLITest{
		...
		{args: "stream-share provider share describe ss-12345", fixture: "stream-share/describe-provider-share.golden"},
		{
			...
			args:        "stream-share provider share delete ss-12345",
			fixture:     "stream-share/delete-provider-share-accept.golden",
		},
		{
			preCmdFuncs: []bincover.PreCmdFunc{stdinPipeFunc(strings.NewReader("n\n"))},
			args:        "stream-share provider share delete ss-12345",
			fixture:     "stream-share/delete-provider-share-reject.golden",
		},
