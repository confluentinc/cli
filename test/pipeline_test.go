...
func (s *CLITestSuite) TestSDPipeline() {
	...
	tests := []CLITest{
		...
		{args: fmt.Sprintf("pipeline save pipe-12345 --source-code-sql-file %s", testOutputFile.Name()), fixture: "pipeline/save.golden", regex: true},
		...
		{args: fmt.Sprintf("pipeline create --name testPipeline --ksql-cluster lksqlc-12345 --description testDescription --source-code-sql-file %s", testPipelineSourceCode), fixture: "pipeline/create.golden"},
		...
		{args: fmt.Sprintf("pipeline update pipeline-12345 --source-code-sql-file %s", testPipelineSourceCode), fixture: "pipeline/update.golden"},
