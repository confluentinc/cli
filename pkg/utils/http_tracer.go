...
func GetCloudTracedContext() context.Context {
	ctx := context.Background()
	if log.CliLogger.Level >= log.TRACE {
		return cloudHttpTracedContext(ctx)
	}
	return ctx
}
...
// Note: This variant logs more outputs compared with httpTracedContext but loses the spew.Dump call
// This variant is intended to trace only what we need to diagnose issues w/ Cloud v1 endpoints without changing the on-prem logging
func cloudHttpTracedContext(ctx context.Context) context.Context {
	trace := &httptrace.ClientTrace{
		DNSStart: func(dnsInfo httptrace.DNSStartInfo) {
			log.CliLogger.Tracef("DNS Start; Info: %+v\n", dnsInfo)
		},
		...
		GotConn: func(connInfo httptrace.GotConnInfo) {
			log.CliLogger.Tracef("Got Conn; Info: addr=%s Reused=%t WasIdle=%t IdleTime=%s\n", connInfo.Conn.RemoteAddr(), connInfo.Reused, connInfo.WasIdle, connInfo.IdleTime)
		},
		GetConn: func(hostPort string) {
			log.CliLogger.Tracef("Get Conn; Info: %+v\n", hostPort)
		},
		GotFirstResponseByte: func() {
			log.CliLogger.Trace("Got First Response Byte\n")
		},
		WroteRequest: func(reqInfo httptrace.WroteRequestInfo) {
			if reqInfo.Err != nil {
				log.CliLogger.Tracef("Wrote Request; Error: %+v\n", reqInfo.Err)
			} else {
				log.CliLogger.Trace("Wrote Request\n")
			}
		},
	}
