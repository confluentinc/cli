...
var (
	...
	ConnectionTypeSecretMapping = map[string][]string{
		...
		"rest":           {"username", "password", "token", "token-endpoint", "client-id", "client-secret", "scope", "no-auth"},
		"mcp_server":     {"api-key", "token", "token-endpoint", "client-id", "client-secret", "scope", "no-auth"},
	}
