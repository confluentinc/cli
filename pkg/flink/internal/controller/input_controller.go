...
func NewInputController(history *history.History, lspCompleter prompt.Completer, handlerCh chan *jsonrpc2.Request, cloud bool) types.InputControllerInterface {
	...
	if prompt, err := inputController.initPrompt(cloud); err == nil {
...
func (c *InputController) initPrompt(cloud bool) (prompt.IPrompt, error) {
	...
	return prompt.New(
		nil,
		c.promptCompleter(cloud),
		options...,
	)
}

func (c *InputController) promptCompleter(cloud bool) prompt.Completer {
