...
func (c *command) store(cmd *cobra.Command, args []string) error {
	...
	if err == nil && clusterId != "" {
		...
		if err != nil {
			...
			return errors.NewErrorWithSuggestions(errors.NoKafkaSelectedErrorMsg, apiKeyNotValidForClusterSuggestions)
		}
	}
	...
	if !apiKeyIsValidForTargetCluster {
		return errors.NewErrorWithSuggestions("the provided API key does not belong to the target cluster", apiKeyNotValidForClusterSuggestions)
	}
	...
	if found, err := c.keystore.HasAPIKey(key, cluster.ID); err != nil {
		...
		return errors.NewErrorWithSuggestions(fmt.Sprintf(`refusing to overwrite existing secret for API Key "%s"`, key),
