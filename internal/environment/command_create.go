...
func (c *command) addStreamGovernancePackageFlag(cmd *cobra.Command, defaultValue string) {
	...
	cmd.Flags().String("governance-package", defaultValue, fmt.Sprintf("Specify the Stream Governance package as %s. Note: downgrading the package from \"advanced\" to \"essentials\" is forbidden.", values))
}
