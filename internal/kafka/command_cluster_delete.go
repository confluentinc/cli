...
func (c *clusterCommand) delete(cmd *cobra.Command, args []string) error {
	...
	if errs.ErrorOrNil() != nil {
		if suggestion := deletionProtectionErrorToSuggestion(deletionProtectionDetail); suggestion != "" {
			return errors.NewErrorWithSuggestions(err.Error(), suggestion)
		}
...
func parseDeletionProtectionErrDetail(r *http.Response) (string, bool) {
	...
	if len(res.Errors) > 0 && res.Errors[0].Code == errorCodeDeletionProtectionEnabled {
		return res.Errors[0].Detail, true
	}
