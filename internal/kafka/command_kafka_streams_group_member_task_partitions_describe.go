...
func (c *streamsGroupCommand) newStreamsGroupMemberTaskPartitionsDescribeCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "describe <member>",
		Short: "Describe a Kafka streams group member task partitions.",
...
func (c *streamsGroupCommand) streamsGroupMemberTaskPartitionsDescribe(cmd *cobra.Command, args []string) error {
	...
	taskPartitions, err := kafkaREST.CloudClient.GetKafkaStreamsGroupMemberAssignmentTaskPartitions(groupId, memberId, assignmentType, subtopologyId)
	if err != nil {
		return err
	}
