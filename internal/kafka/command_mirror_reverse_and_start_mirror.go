...
func (c *mirrorCommand) newReverseAndStartMirrorCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:               "reverse-and-start-mirror <destination-topic-1> [destination-topic-2] ... [destination-topic-N]",
		...
		Example: examples.BuildExampleString(
			examples.Example{
				...
				Code: "confluent kafka mirror reverse-and-pause-mirror my-topic-1 my-topic-2 --link my-link",
			},
		),
	}

	pcmd.AddLinkFlag(cmd, c.AuthenticatedCLICommand)
	cmd.Flags().Bool(dryrunFlagName, false, "If set, does not actually reverse and pause the mirror topic, but simply validates it.")
...
func (c *mirrorCommand) reverseAndPauseMirror(cmd *cobra.Command, args []string) error {
	...
	results, err := kafkaREST.CloudClient.UpdateKafkaMirrorTopicsReverseAndPauseMirror(link, dryRun, alterMirrorsRequestData)
	if err != nil {
		return err
	}
