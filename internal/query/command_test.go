...
// stringValueForTest is a minimal pflag.Value so TestResolveEnvironmentAlias can
// register two flag names against one shared value, the same way addCatalogAlias
// does for the real "environment"/"catalog" flags.
...
func TestResolveDatabase(t *testing.T) {
	...
	got, err := resolveDatabase(newDBCmd("", ""))
	require.NoError(t, err)
	require.Empty(t, got)

	got, err = resolveDatabase(newDBCmd("lkc-database", ""))
	require.NoError(t, err)
	require.Equal(t, "lkc-database", got)

	got, err = resolveDatabase(newDBCmd("", "lkc-cluster"))
	require.NoError(t, err)
	require.Equal(t, "lkc-cluster", got)

	_, err = resolveDatabase(newDBCmd("lkc-database", "lkc-cluster"))
