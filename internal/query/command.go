...
func (c *command) runQuery(cmd *cobra.Command, _ []string) error {
	// Registered before any other work, including flag validation, to shrink the
	// window where a Ctrl-C predates any signal handling in this process and
	// falls through to the OS default disposition instead (silent kill, no exit
	// code or message this package controls). This can't close that window
	// entirely: PersistentPreRunE's own network calls (token refresh, the
	// update-available check) run before RunE — and this line — even start. It
	// only removes the extra slice of that window that used to also elapse
	// inside RunE itself before a handler was installed.
