...
// land and calls cleanup instead of leaking an unstoppable statement. The returned
// bool is cleanup's result (false, without being called, if create never landed
// within gracePeriod) so the caller can fold the outcome into one message instead
// of cleanup also announcing it separately.
...
// nothing to name or check and stopped is ignored. Otherwise stopped reports
// whether the caller already stopped the statement.
//
// A timeout (--wait-timeout elapsed) is always reported as "Error:" — it isn't
// something the user asked for. Ctrl-C is different: the user asked for it, so
// none of its outcomes are reported as "Error:" — all three (nothing had been
// created yet, the statement was confirmed stopped, or the stop couldn't be
// confirmed) are reported as "Interrupted:" instead, via cmd directly.
func interruptedError(cmd *cobra.Command, err error, name string, stopped bool) error {
	...
	return errors.NewErrorWithSuggestions(
		msg,
		fmt.Sprintf("Stop it with `confluent flink statement stop %s`.", name),
	)
}
...
func (c *command) handleQueryError(cmd *cobra.Command, client *ccloudv2.FlinkGatewayClient, environmentId, name string, err error, settled *bool) error {
	...
	if goerrors.As(err, &unbounded) {
		...
		fate := fmt.Sprintf("Statement \"%s\" was stopped.", name)
		if !c.stopStatement(client, environmentId, name, false) {
			fate = fmt.Sprintf("Stop it with `confluent flink statement stop %s`.", name)
		}
		*settled = true
		return errors.NewErrorWithSuggestions(
