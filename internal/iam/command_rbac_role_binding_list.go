...
func (c *roleBindingCommand) newListCommand() *cobra.Command {
	...
	cmd.Flags().String("resource", "", "Resource type and identifier using `<Prefix>:<ID>` format (for example, `User:u-123456`). If specified with `--role` and no principals, list all principals and role bindings  the `--role` for the specified resource.")
