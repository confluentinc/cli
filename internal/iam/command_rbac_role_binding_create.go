...
func (c *roleBindingCommand) newCreateCommand() *cobra.Command {
	...
	if c.cfg.IsCloudLogin() {
		...
		if c.cfg.IsTest || featureflags.Manager.BoolVariation("cli.flink.open_preview", c.Context, config.CliLaunchDarklyClient, true, false) {
			exs = append(exs,
				examples.Example{
					Text: `Grant the role "FlinkDeveloper" to the principal "User:u-123456", in the environment "env-123456":`,
					Code: "confluent iam rbac role-binding create --principal User:u-123456 --role FlinkDeveloper --environment env-123456",
				},
				examples.Example{
					Text: `Grant the role "FlinkDeveloper" to the principal "User:u-123456", in the environment "env-123456" for the compute pool "lfcp-123456" in the Flink region "us-east-2":`,
