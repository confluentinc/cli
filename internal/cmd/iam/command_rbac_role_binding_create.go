...
func (c *roleBindingCommand) newCreateCommand() *cobra.Command {
	...
	if c.cfg.IsCloudLogin() {
		cmd.Example = examples.BuildExampleString(
			...
			examples.Example{
				Text: `Grant the role "KsqlAdmin" to the principal "User:u-123456", in the environment "env-12345" and cloud cluster "lkc-123456" for the KSQL cluster named "my-ksql-cluster":`,
				Code: "confluent iam rbac role-binding create --principal User:u-123456 --role KsqlAdmin --environment env-12345 --cloud-cluster lkc-123456 --ksql-cluster my-ksql-cluster",
			},
		)
	} else {
		cmd.Example = examples.BuildExampleString(
