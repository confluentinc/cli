...
func (c *command) newCreateCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: examples.BuildExampleString(
			examples.Example{
				Text: `Create an API key for service account with resource ID "sa-123456" for cluster "lkc-123456"`,
...
func (c *command) create(cmd *cobra.Command, _ []string) error {
	...
	if err := output.DescribeObject(cmd, userKey, createFields, createHumanRenames, createStructuredRenames); err != nil {
		return err
	}
