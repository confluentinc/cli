...
func (c *Command) logout(cmd *cobra.Command, _ []string) error {
	if c.Config.Config.Context() != nil {
		username, err := c.netrcHandler.RemoveNetrcCredentials(c.cfg.IsCloudLogin(), c.Config.Config.Context().LoginContext)
		if err == nil {
		...
		if runtime.GOOS == "darwin" && !c.cfg.IsTest {
			if err := keychain.Delete(c.Config.Config.Context().LoginContext); err != nil {
				return err
			}
		}
	}
