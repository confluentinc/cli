...
func (c *mirrorCommand) failover(cmd *cobra.Command, args []string) error {
	...
	req := kafkaREST.Client.ClusterLinkingV3Api.UpdateKafkaMirrorTopicsFailover(kafkaREST.Context, lkc, linkName)
	req = req.AlterMirrorsRequestData(cloudkafkarest.AlterMirrorsRequestData{MirrorTopicNames: args}).ValidateOnly(validateOnly)
