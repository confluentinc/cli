...
func (c *mirrorCommand) newCreateCommand() *cobra.Command {
	...
	cmd.Flags().String(sourceTopicFlagName, "", "Name of the topic to be mirrored over the cluster link, that is, the source topic's name. Only required when there is a prefix configured on the link.")
