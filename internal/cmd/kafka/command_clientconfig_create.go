...
const (
	...
	ContextExample = "confluent kafka client-config create %s"
	FlagExample    = "confluent kafka client-config create %s --environment env-123 --cluster lkc-123456 --api-key my-key --api-secret my-secret"
...
var (
	...
	Go         = &clientConfig{"Go", "go", LibrdKafkaConfig, false}
...
func (c *createCommand) init() {
	...
	c.addCommand(Rust)
	c.addCommand(Scala)
	c.addCommand(SpringBoot)
}

func (c *createCommand) addCommand(clientConfig *clientConfig) {
	contextExample := fmt.Sprintf(ContextExample, clientConfig.languageId)
	flagExample := fmt.Sprintf(FlagExample, clientConfig.languageId)
	...
	cmd := &cobra.Command{
		...
		Short: fmt.Sprintf(ClientConfigDescriptionFmt, clientConfig.language) + ".",
		Long: fmt.Sprintf(ClientConfigDescriptionFmt, clientConfig.language) + ", of which the client configuration file " +
			"is printed to stdout and the warnings is printed to stderr. Please see our examples on how " +
			"to redirect the command output.",
		Args:        cobra.NoArgs,
		RunE:        pcmd.NewCLIRunE(c.create(clientConfig.configId, clientConfig.srApiAvailable)),
		Annotations: map[string]string{pcmd.RunRequirement: pcmd.RequireCloudLogin},
		Example: examples.BuildExampleString(
			examples.Example{
				Text: fmt.Sprintf(ClientConfigDescriptionFmt, clientConfig.language) + ".",
				Code: contextExample,
			},
			examples.Example{
				Text: fmt.Sprintf(ClientConfigDescriptionFmt, clientConfig.language) + " with arguments passed via flags.",
				Code: flagExample,
			},
			examples.Example{
				Text: fmt.Sprintf(ClientConfigDescriptionFmt, clientConfig.language) + ", redirecting the configuration to a file and the warnings to a separate file.",
				Code: fmt.Sprintf(ContextExample, clientConfig.languageId) + " 1> my-client-config-file.config 2> my-warnings-file",
			},
			examples.Example{
				Text: fmt.Sprintf(ClientConfigDescriptionFmt, clientConfig.language) + ", redirecting the configuration to a file and keeping the warnings in the console.",
				Code: fmt.Sprintf(ContextExample, clientConfig.languageId) + " 1> my-client-config-file.config 2>&1",
			},
		),
	}
...
func (c *createCommand) create(configId string, srApiAvailable bool) func(cmd *cobra.Command, _ []string) error {
	return func(cmd *cobra.Command, _ []string) error {
		...
		configFile, err := c.fetchConfigFile(configId)
		if err != nil {
			return err
		}
...
func (c *createCommand) fetchConfigFile(configId string) ([]byte, error) {
...
func (c *createCommand) setKafkaCluster(cmd *cobra.Command, configFile []byte) ([]byte, error) {
	...
	kafkaCluster, err := c.getKafkaCluster()
	if err != nil {
		return nil, err
	}
...
func (c *createCommand) getKafkaCluster() (*v1.KafkaClusterConfig, error) {
