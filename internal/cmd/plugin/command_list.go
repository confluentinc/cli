...
func (c *command) list(cmd *cobra.Command, _ []string) error {
	...
	if len(pluginMap) == 0 && output.GetFormat(cmd) == output.Human {
		utils.ErrPrintln("Please run `confluent plugin -h` for information on how to make plugins discoverable by the CLI.")
	}
	...
	for _, pluginInfo := range nameConflictPlugins {
		utils.ErrPrintf("[WARN] The built-in command `%s` will be run instead of the duplicate plugin at %s.\n", pluginInfo.PluginName, pluginInfo.FilePath)
	}
	for _, pluginInfo := range overshadowedPlugins {
		utils.ErrPrintf("[WARN] The command `%s` will run the plugin listed above instead of the duplicate plugin at %s.\n", pluginInfo.PluginName, pluginInfo.FilePath)
	}
