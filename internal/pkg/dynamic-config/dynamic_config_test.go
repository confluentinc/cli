...
func TestDynamicConfig_ParseFlagsIntoConfig(t *testing.T) {
	...
	dynamicConfigBase := New(config, pmock.NewClientMock(), pmock.NewPublicClientMock(), pmock.NewV2ClientMock())

	config = v1.AuthenticatedCloudConfigMock()
	dynamicConfigFlag := New(config, pmock.NewClientMock(), pmock.NewPublicClientMock(), pmock.NewV2ClientMock())
