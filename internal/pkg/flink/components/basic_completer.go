...
func EXAMPLESCompleter(in prompt.Document) []prompt.Suggest {
	s := []prompt.Suggest{
		...
		{Text: "ANALYZE TABLE Orders COMPUTE STATISTICS;", Description: "Used to collect statistics for existing tables and store the result to catalog"},
		{Text: "DESCRIBE Orders;", Description: "Describe the schema of a table or a view"},
		{Text: "EXPLAIN PLAN FOR ...;", Description: "Used to explain the logical and optimized query plans of a query or an INSERT statement"},
