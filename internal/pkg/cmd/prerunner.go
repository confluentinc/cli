	...
	if requirement, ok := cmd.Annotations[RunRequirement]; ok {
		...
		case RequireNonAPIKeyCloudLogin:
			if !(cfg.CredentialType() != v2.APIKey && cfg.IsCloudLogin()) {
				return errors.NewErrorWithSuggestions(
					"you must log in to Confluent Cloud with a username and password to run this command",
		...
		case RequireNonAPIKeyCloudLoginOrOnPremLogin:
			if !(cfg.CredentialType() != v2.APIKey && cfg.IsCloudLogin() || cfg.IsOnPremLogin()) {
				return errors.NewErrorWithSuggestions(
					"you must log in to Confluent Cloud with a username and password or log in to Confluent Platform to run this command",
		...
		case RequireCloudLogin:
			if !cfg.IsCloudLogin() {
				return errors.NewErrorWithSuggestions(
					"you must log in to Confluent Cloud to run this command",
					`Log in with "confluent login"`,
				)
			}
		case RequireCloudLoginOrOnPremLogin:
			if !(cfg.IsCloudLogin() || cfg.IsOnPremLogin()) {
				return errors.NewErrorWithSuggestions(
					"you must log in to run this command",
					`Log in with "confluent login"`,
				)
			}
		case RequireOnPremLogin:
			if !cfg.IsOnPremLogin() {
				return errors.NewErrorWithSuggestions(
					"you must log in to Confluent Platform run this command",
		...
		case RequireUpdatesEnabled:
			if cfg.DisableUpdates {
				return errors.NewErrorWithSuggestions(
					"you must enable updates to run this command",
		...
		if err := r.setAuthenticatedWithMDSContext(cmd, command); err != nil {
			if _, ok := err.(*errors.NotLoggedInError); ok {
				if autoLoginErr := r.confluentAutoLogin(cmd); autoLoginErr != nil {
					r.Logger.Debugf("Auto login failed: %s", autoLoginErr.Error())
					return err
				}
				if err := r.setAuthenticatedWithMDSContext(cmd, command); err != nil {
					return err
