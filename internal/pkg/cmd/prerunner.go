...
func (r *PreRun) setCCloudClient(cliCmd *AuthenticatedCLICommand) error {
	...
	if cliCmd.KafkaRESTClient != nil {
		// This is set here to allow it to be mocked. In the KafkaAPI case,
		// these calls happen inside each ccloud-sdk-go. kafka-rest-sdk-go
		// is completely autogenerated, so the same approach cannot be taken
		// here.
		cliCmd.KafkaRESTBearerToken = r.getBearerToken(ctx, cliCmd)
	}
	return nil
}
...
func (r *PreRun) getBearerToken(ctx *DynamicContext, cliCmd *AuthenticatedCLICommand) string {
