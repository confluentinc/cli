...
func (r *PreRun) printWarnings(cmd *cobra.Command) {
	...
	for _, warning := range []struct {
		...
		{prefix: "ccloud kafka acl", text: fmt.Sprintf(fmtBreakingChange, "the following output labels/keys were renamed: ServicAccountId to Principal, Resource to ResourceType, Name to ResourceName, and Type to PatternType.")},
		...
		{prefix: "ccloud kafka topic describe", text: fmt.Sprintf(fmtBreakingChange, "topic parition and replica data has been removed.")},
