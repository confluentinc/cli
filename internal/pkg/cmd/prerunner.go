...
func (r *PreRun) printWarnings(cmd *cobra.Command) {
	...
	for _, warning := range []struct {
		...
		{prefix: "ccloud", text: "The Confluent Cloud CLI is deprecated and will stop working on May 9, 2022. All `ccloud` features have been moved to the Confluent CLI: https://docs.confluent.io/confluent-cli/current/index.html\nTo update to the new CLI, run `ccloud update --major`. See the migration guide for more details: https://docs.confluent.io/ccloud-cli/current/migrate.html\n"},
		...
		{prefix: "ccloud kafka acl", text: fmt.Sprintf(fmtBreakingChange, "the names of the flags have been updated to match those of the Confluent REST Proxy.")},
		...
		{prefix: "confluent iam acl", text: fmt.Sprintf(fmtBreakingChange, "the names of the flags have been updated to match those of the Confluent REST Proxy.")},
