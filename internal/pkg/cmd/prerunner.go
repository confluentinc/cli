...
func (r *PreRun) setCCloudClient(cliCmd *AuthenticatedCLICommand) error {
	...
	if os.Getenv("XX_CCLOUD_DO_NOT_USE_KAFKA_REST") == "" {
		...
		cliCmd.KafkaREST.Client, err = r.createKafkaRESTClient(ctx, cliCmd)
		if err != nil {
			return err
		}
		// This is set here to allow it to be mocked. In the KafkaAPI case, these
		// calls happen inside each ccloud-sdk-go. kafka-rest-sdk-go is completely
		// autogenerated, so the same approach cannot be taken here.
		cliCmd.KafkaREST.BearerToken, err = r.getBearerToken(ctx, cliCmd)
		if err != nil {
			return err
		}
	}
...
func (r *PreRun) createKafkaRESTClient(ctx *DynamicContext, cliCmd *AuthenticatedCLICommand) (*kafkarestv3.APIClient, error) {
...
func (r *PreRun) getBearerToken(ctx *DynamicContext, cliCmd *AuthenticatedCLICommand) (string, error) {
	state, err := ctx.AuthenticatedState(cliCmd.Command)
	if err != nil {
		return "", err
	}
	accessToken, err := getBearerToken(state, ctx.Platform.Server)
	if err != nil {
		return "", err
	}
