...
func (n *NetrcHandler) WriteNetrcCredentials(cliName string, isSSO bool, ctxName string, username string, password string) error {
	...
	if err != nil {
		return errors.NewUnexpectedCLIBehaviorErrorWrapf(err, errors.AuthNetrcHandlerResolvingFilepathErrorMsg, filename)
	}

	netrcFile, err := getOrCreateNetrc(filename)
	if err != nil {
		return errors.NewUnexpectedCLIBehaviorErrorWrapf(err, errors.AuthNetrcHandlerWriteToNetrcFileErrorMsg, filename)
	}
...
func (n *NetrcHandler) getNetrcCredentials(cliName string, isSSO bool, ctxName string) (username string, password string, err error) {
	...
	if err != nil {
		return "", "", errors.NewUnexpectedCLIBehaviorErrorWrapf(err, errors.AuthNetrcHandlerGetNetrcCredentialsErrorMsg, filename)
	}
	if machine == nil {
		return "", "", errors.NewResourceValidationErrorf(errors.AuthNetrcHandlerCredentialsNotFoundErrorMsg, filename)
	}
...
func getOrCreateNetrc(filename string) (*netrc.Netrc, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			if err != nil {
				return nil, errors.NewUnexpectedCLIBehaviorErrorWrapf(err, errors.AuthNetrcHandlerCreateFileErrorMsg, filename)
			}
			n, err = netrc.ParseFile(filename)
			if err != nil {
				return nil, err
