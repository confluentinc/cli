...
type MDSClientManager interface {
	GetMDSClient(ctx *v3.Context, caCertPath string, caCertParthFlagChagned bool, url string, logger *log.Logger) (*mds.APIClient, error)
}

type MDSClientManagerImpl struct{}

func (m *MDSClientManagerImpl) GetMDSClient(ctx *v3.Context, caCertPath string, caCertParthFlagChagned bool, url string, logger *log.Logger) (*mds.APIClient, error) {
	mdsClient := initializeMDS(ctx, logger)
	if logger.GetLevel() == log.DEBUG || logger.GetLevel() == log.TRACE {
		mdsClient.GetConfig().Debug = true
	}
	if caCertParthFlagChagned {
		if caCertPath == "" {
