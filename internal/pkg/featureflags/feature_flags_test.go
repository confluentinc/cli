...
func (suite *LaunchDarklyTestSuite) TestFlags() {
	...
	boolFlag := ld.BoolVariation("testBool", ctx, CliClientType, false)
	req.Equal(true, boolFlag)

	stringFlag := ld.StringVariation("testString", ctx, CliClientType, "")
	req.Equal("string", stringFlag)

	intFlag := ld.IntVariation("testInt", ctx, CliClientType, 5)
	req.Equal(1, intFlag)

	jsonFlag := ld.JsonVariation("testJson", ctx, CliClientType, map[string]string{})
...
func (suite *LaunchDarklyTestSuite) TestBoolVariation() {
	req := require.New(suite.T())
	ld := launchDarklyManager{}
	evaluatedFlag := ld.BoolVariation("testBool", suite.ctx, CliClientType, false)
...
func (suite *LaunchDarklyTestSuite) TestIntVariation() {
	req := require.New(suite.T())
	ld := launchDarklyManager{}
	evaluatedFlag := ld.IntVariation("testInt", suite.ctx, CliClientType, 0)
...
func (suite *LaunchDarklyTestSuite) TestStringVariation() {
	req := require.New(suite.T())
	ld := launchDarklyManager{}
	evaluatedFlag := ld.StringVariation("testString", suite.ctx, CliClientType, "")
...
func (suite *LaunchDarklyTestSuite) TestJsonVariation() {
	...
	evaluatedFlag := ld.JsonVariation("testJson", suite.ctx, CliClientType, nil)
