...
func (suite *LaunchDarklyTestSuite) TestFlags() {
	...
	boolFlag := ld.BoolVariation("testBool", ctx, CliLaunchDarklyClient, false)
	req.Equal(true, boolFlag)

	stringFlag := ld.StringVariation("testString", ctx, CliLaunchDarklyClient, "")
	req.Equal("string", stringFlag)

	intFlag := ld.IntVariation("testInt", ctx, CliLaunchDarklyClient, 5)
	req.Equal(1, intFlag)

	jsonFlag := ld.JsonVariation("testJson", ctx, CliLaunchDarklyClient, map[string]string{})
...
func (suite *LaunchDarklyTestSuite) TestBoolVariation() {
	req := require.New(suite.T())
	ld := launchDarklyManager{}
	evaluatedFlag := ld.BoolVariation("testBool", suite.ctx, CliLaunchDarklyClient, false)
...
func (suite *LaunchDarklyTestSuite) TestIntVariation() {
	req := require.New(suite.T())
	ld := launchDarklyManager{}
	evaluatedFlag := ld.IntVariation("testInt", suite.ctx, CliLaunchDarklyClient, 0)
...
func (suite *LaunchDarklyTestSuite) TestStringVariation() {
	req := require.New(suite.T())
	ld := launchDarklyManager{}
	evaluatedFlag := ld.StringVariation("testString", suite.ctx, CliLaunchDarklyClient, "")
...
func (suite *LaunchDarklyTestSuite) TestJsonVariation() {
	...
	evaluatedFlag := ld.JsonVariation("testJson", suite.ctx, CliLaunchDarklyClient, nil)
