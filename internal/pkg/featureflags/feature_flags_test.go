...
func (suite *LaunchDarklyTestSuite) TestFlags() {
	...
	boolFlag := ld.BoolVariation("testBool", ctx, v1.CliLaunchDarklyClient, false)
	req.Equal(true, boolFlag)

	stringFlag := ld.StringVariation("testString", ctx, v1.CliLaunchDarklyClient, "")
	req.Equal("string", stringFlag)

	intFlag := ld.IntVariation("testInt", ctx, v1.CliLaunchDarklyClient, 5)
	req.Equal(1, intFlag)

	jsonFlag := ld.JsonVariation("testJson", ctx, v1.CliLaunchDarklyClient, map[string]string{})
...
func (suite *LaunchDarklyTestSuite) TestBoolVariation() {
	req := require.New(suite.T())
	ld := launchDarklyManager{}
	evaluatedFlag := ld.BoolVariation("testBool", suite.ctx, v1.CliLaunchDarklyClient, false)
...
func (suite *LaunchDarklyTestSuite) TestIntVariation() {
	req := require.New(suite.T())
	ld := launchDarklyManager{}
	evaluatedFlag := ld.IntVariation("testInt", suite.ctx, v1.CliLaunchDarklyClient, 0)
...
func (suite *LaunchDarklyTestSuite) TestStringVariation() {
	req := require.New(suite.T())
	ld := launchDarklyManager{}
	evaluatedFlag := ld.StringVariation("testString", suite.ctx, v1.CliLaunchDarklyClient, "")
