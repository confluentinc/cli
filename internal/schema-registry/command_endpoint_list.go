...
func (c *command) endpointList(cmd *cobra.Command, _ []string) error {
	...
	networks, err := c.V2Client.ListNetworks(environmentId, nil, []string{cloud.Aws, cloud.Azure}, []string{}, nil, []string{"READY"}, nil)
	if err != nil {
		return fmt.Errorf("unable to list Schema Registry endpoints: failed to list AWS and Azure networks: %w", err)
	}
