...
func (c *kafkaClusterCommand) newDeleteCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short:             "Deregister one or more USM Kafka clusters.",
		...
		Example: examples.BuildExampleString(
			examples.Example{
				...
				Code: "confluent usm kafka-cluster deregister usmkc-abc123",
			},
		),
	}
...
func (c *kafkaClusterCommand) delete(cmd *cobra.Command, args []string) error {
	...
	existenceFunc := func(primaryId string) bool {
		_, _, err := c.V2Client.GetUsmKafkaCluster(primaryId, environmentId)
		return err == nil
	}
